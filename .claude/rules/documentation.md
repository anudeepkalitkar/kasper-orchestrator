# Rule: Documentation — durable system docs in `docs/`, ephemeral task docs in `tasks/`
...
Don't mix the two. Drift is a bug in both.**
...
docs/
  ...
  adr/              # one file per significant decision, with the why
...
1. **Docs are living.** A change that alters architecture, a flow, a deploy step, or a documented
   decision updates the doc **in the same PR**. A doc that contradicts the code is a defect.
...
### ADRs — record every significant, hard-to-reverse decision
...
ADR trivial choices. Naming: `docs/adr/NNNN-short-title.md`, zero-padded, monotonic. **Append-only**:
to change an accepted decision, write a new ADR that supersedes it and mark the old one
`Superseded by ADR-NNNN` — the trail of why things changed is the value. Template:
```
...
- New project: create `docs/ARCHITECTURE.md`, `docs/adr/`, and (with a UI) `docs/design/` during
  foundations, before feature code.
- Every PR that changes system behavior: check whether ARCHITECTURE, a design spec, a runbook, or
  an ADR needs updating — same PR.
