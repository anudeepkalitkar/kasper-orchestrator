...
The tester and code-reviewer seats are not Claude subagents: KASPER invokes this
script as a single Bash call, and Codex does the work headless. The seat's
discipline lives in ``.claude/codex/<seat>.md``; the task-specific brief is a file
KASPER writes. The prompt handed to Codex is the two concatenated.
...
def build_prompt(seat: Seat, brief: Path) -> str:
    """Compose the prompt for a seat: its role prompt, then the task brief.
    ...
    Raises:
        OSError: The role prompt or the brief could not be read.
    """
    role = (ROLE_PROMPT_DIR / f"{seat}.md").read_text(encoding="utf-8")
    return f"{role.rstrip()}\n\n## Brief\n{brief.read_text(encoding='utf-8').strip()}\n"
...
def codex_argv(seat: Seat, out: Path, prompt: str) -> list[str]:
    ...
    which is the only shape that accepts our rules as a prompt.
...
def main(argv: list[str]) -> int:
    ...
    seat: Seat = args.seat  # argparse `choices` guarantees one of the two literals
    try:
        return run_seat(seat, args.brief, args.out, args.timeout, args.dry_run)
    finally:
        print(args.out)
