...
def _session_end_handlers(home: Path) -> list[dict[str, Any]]:
    """Every ``SessionEnd`` handler in the installed settings, in document order.

    Args:
        home: The Claude home to read.

    Returns:
        The handler dicts, or an empty list when the event is not there at all.
    """
    groups: list[dict[str, Any]] = _settings(home).get("hooks", {}).get("SessionEnd", [])
